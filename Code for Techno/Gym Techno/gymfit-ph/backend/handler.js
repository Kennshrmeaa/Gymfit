...
function planDefinition(plan) {
  if (plan === 'annual') {
    return { plan: 'annual', label: 'Premium Annual', amountMinor: 79900, years: 1 };
  }
...
async function handleSendOtp(body) {
  ...
  if (existing && existing.cooldownUntil > now) {
    return fail(429, `Please wait ${Math.ceil((existing.cooldownUntil - now) / 1000)}s before requesting another code.`);
  }
  ...
  return json({ ok: true, cooldownInSec: 60, expiresInSec: 300 });
}
...
async function handleVerifyOtp(body) {
  ...
  if (!isValidEmail(email)) return fail(400, 'Enter a valid email address.');
  if (!/^\d{6}$/.test(code)) return fail(400, 'Enter the 6-digit verification code.');

  pruneStore();
  const record = store.otpRequests[email];
  if (!record) return fail(400, 'No verification code is waiting for this email. Request a new one first.');
  if (record.expiresAt < Date.now()) {
    delete store.otpRequests[email];
    saveStore();
    return fail(400, 'That verification code already expired. Request a new one.');
  }

  if (hashOtp(email, code) !== record.codeHash) {
    record.attempts = (record.attempts || 0) + 1;
    saveStore();
    return fail(400, 'The verification code is incorrect.');
  }

  delete store.otpRequests[email];
  store.verifiedEmails[email] = { verifiedAt: nowIso() };
  saveStore();
  return json({ ok: true, verifiedAt: store.verifiedEmails[email].verifiedAt });
}
...
async function handleCreateCheckout(body) {
  ...
  if (!isValidEmail(email)) return fail(400, 'Enter a valid customer email before starting checkout.');
  if (!['gcash', 'maya', 'card', 'bank'].includes(preferredMethod)) {
    return fail(400, 'Unsupported payment method selection.');
  }
...
async function handleSubscriptionStatus(searchParams) {
...
async function handlePaymongoWebhook(rawBody) {
  ...
  // Signature would need to be passed separately from req.headers
...
export async function handleRequest(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/auth/send-otp') {
    ...
    if (req.method === 'POST' && pathname === '/subscriptions/create-checkout') {
      const result = await handleCreateCheckout(body);
      res.statusCode = result.status;
      res.end(result.body);
      return;
    }

    if (req.method === 'GET' && pathname === '/subscriptions/status') {
      const result = await handleSubscriptionStatus(url.searchParams);
      res.statusCode = result.status;
      res.end(result.body);
      return;
    }

    if (req.method === 'POST' && pathname === '/paymongo/webhook') {
      const chunks = [];
      for await (const chunk of req) {
        chunks.push(chunk);
      }
    ...
    const result = fail(404, 'Endpoint not found.');
    res.statusCode = result.status;
    res.end(result.body);
  } catch (err) {
    console.error('API error:', err);
    const result = fail(500, err.message || 'Internal server error.');
    res.statusCode = result.status;
    res.end(result.body);
  }
}
